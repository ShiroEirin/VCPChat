...
const term = new Terminal({
  cursorBlink: true,
  fontSize: 14,
  // Nerd Font 优先：oh-my-posh 等 prompt 主题的图标(U+E0B0 区段)依赖 Nerd Font；
  // 无 NF 字体时依次回退到系统等宽字体。
  fontFamily:
    '"Maple Mono Normal NF CN", "Cascadia Code", Consolas, "Courier New", monospace',
  theme: {},
...
function fitTerminal() {
  try {
    ...
    if (window.electronAPI) {
      window.electronAPI.send("powershell-resize", {
        cols: term.cols,
        rows: term.rows,
      });
    }
